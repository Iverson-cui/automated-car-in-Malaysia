...
//		oled_delayus(1);
		OLED_SCL = 1;
//		oled_delayus(1);  //必须有保持SCL脉冲的延时
		OLED_SCL = 0;
//		oled_delayus(1);
		IIC_Byte <<= 1;
	}
    OLED_SDA = 1; //原程序这里有一个拉高SDA，根据OLED的DATASHEET，此句必须去掉。
                  //no no no,现在就该加上了
	OLED_SCL = 1;
	oled_delayus(1);
	OLED_SCL = 0;
...
{   
	/*************************************************/
	OLED_RST = 0;
	oled_delayms(100);//初始化之前的延时很重要！
	OLED_RST = 1;
	// 这部分在使用32驱动的并没有
	// 但是使用51单片机的必须加上这个复位，先拉低持续200ms,再一直保持高电平
	// 当然，如果OLED上没有这个复位接口，那也许是集成到了板子里，这部分也就不用管了
	// 影响不大
	/*************************************************/
...
显示的坐标（x,y），y为页范围0～7  against:0,反白显示;1,正常显示*/
